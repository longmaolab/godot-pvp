#!/usr/bin/env bash
# Verify listen-host no longer accepts raw remote transform pushes and instead
# moves remote peers from input bits on the host side.
...
mkdir -p "$LOG_DIR"
HOST_LOG="$LOG_DIR/mp_host_collision_guard_host.log"
CLIENT_LOG="$LOG_DIR/mp_host_collision_guard_client.log"
PORT=9233

echo "=== listen-host collision authority guard test ==="

"$GODOT" --headless --path "$PROJ" tests/mp_host_collision_guard_test.tscn \
