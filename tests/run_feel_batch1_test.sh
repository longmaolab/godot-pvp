...
LOG="$LOG_DIR/feel_batch1.log"
echo "=== feel batch1 (death anim + dynamic crosshair) ==="
"$GODOT" --headless --path "$PROJ" -s tests/feel_batch1_test.gd >"$LOG" 2>&1 &
PID=$!
( sleep 40 && kill -9 $PID 2>/dev/null && echo "[killed]" >>"$LOG" ) &
K=$!
