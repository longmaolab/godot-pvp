...
- 本轮复审当前 `HEAD` = `4184241`（近期变更包含 spawn 空隙校验、1.5s 出生无敌、dagger/hammer equipped melee）。
- 新的 melee 单机测试通过，spawn clearance / RoomWorld / DB / main-menu compression 也通过；未发现新的 P0。
- 发现 1 个新的 P1：equipped melee 在 listen-host/direct `client_fire` 路径没有走 melee 专用判定，会被服务器当成 500m hitscan 子弹处理并扣掉假弹匣。旧的 3 个 P2 仍开放。

### [P1] Equipped melee 的 direct RPC 路径仍走 hitscan，listen-host/恶意 RPC 可远距离刀人且第二刀卡弹匣
**文件**：`shared/scripts/player_controller.gd:1459`、`shared/scripts/player_controller.gd:1470`、`server/scripts/fire_resolver.gd:95`、`server/scripts/fire_resolver.gd:174`、`server/scripts/fire_resolver.gd:289`、`server/scripts/fire_resolver.gd:355`

**问题**：新 melee 分支只在 `PlayerController.try_fire()` 的 authority / DS input-bit 路径调用 `_melee_strike(weapon_def.damage, weapon_def.melee_range, ...)`。但非服务器客户端按原有逻辑仍会发 `client_fire`，而 `FireResolver.resolve_fire()` 没有 `weapon.is_melee()` 分支：它先检查 `ammo_in_mag <= 0`，再在第 174 行扣 1 发弹药，然后因为 melee weapon 的 `bullet_speed = 0`、`is_hitscan()` 为 true（`_weapon_def.gd:126`），第 289 行给它 500m raycast，最后第 355 行按普通枪械伤害结算。dagger/hammer 的 `magazine = 1`，所以 direct RPC 路径第一次可以远距离命中，随后服务器弹匣变 0，下一次 swing 会触发 reload/拒绝，而不是按 melee cooldown 工作。

**为什么重要**：Dedicated-server 的正常 input-bit 路径大概率没事，但 listen-host / legacy direct-fire 路径和任意能直接发 `client_fire` 的客户端会把 dagger/hammer 变成长距离 hitscan 武器。这既破坏玩法公平，也让第二次近战表现成“武器突然不能挥”，属于实际 combat/networking regression。现有 `tests/melee_weapon_test.gd` 只覆盖单机/offline `try_fire()`，没有覆盖 `FireResolver` 的 direct RPC 分支。

**建议**：在 `FireResolver.resolve_fire()` 解析并验证 weapon/loadout/current weapon 后、ammo gate/commit/hitscan 前，优先处理 `weapon.is_melee()`：只 arm `time_until_next_shot`，调用服务器侧 melee strike（或抽一个共享 resolver，使用 `melee_range` + hitbox mask），不要扣 `ammo_in_mag`，不要进入 hitscan/falloff。另补一个 FireResolver/listen-host 回归测试：target 超过 `melee_range` 必须无伤，range 内命中后第二次只受 cooldown gate 而不是 ammo gate。
...
- `git diff --stat 9a8e5a8..HEAD`：近期代码变化集中在 spawn reform、invincibility、dagger/hammer melee、HUD ammo display 和新增 melee test。
- `HOME=/private/tmp/godot-home bash tests/run_quick.sh`
  - 9 passed / 1 failed in 13s
  - 唯一失败：`boot_test` 仍误报 macOS `get_system_ca_certificates` 前一行 generic `ERROR`
- `HOME=/private/tmp/godot-home bash tests/run_melee_weapon_test.sh` PASS（dagger/hammer 单机 equipped melee：伤害、无弹药、cooldown、range gate）
...
1. 先修 `FireResolver` 的 `weapon.is_melee()` direct RPC 分支，并补 listen-host/direct resolver 回归测试。
2. 修 boot-test CA 上下文过滤，让 `run_quick.sh` 恢复可信绿灯。
3. 明确 wheel 产品规则并把 cooldown/付费状态改为服务器驱动。
4. 统一默认 loadout 第 4 槽，避免 Reset/Save 覆盖实战默认。
